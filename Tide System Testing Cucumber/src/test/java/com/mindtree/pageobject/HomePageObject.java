...
public class HomePageObject
{
	...
	public void clickSearchBox(WebDriver driver)
	{
		...
		try
		{
			...
			rm.clickIcon(homeObjectLoc.searchBox);
			BaseClass.ts.pass("Clicked the search box");
			l.debug("Clicked the search box");
		}
		catch (Exception e)
		{
			BaseClass.ExecutionFlag = true;
			System.out.println(e.getMessage());
			l.error("Failed to click the search box. Search box not found");
			BaseClass.ts.fail("Failed to click the search box. Search box not found", MediaEntityBuilder.createScreenCaptureFromPath(reportObject.addScreenShot(driver)).build());
		}
	}
